...
export default async function Image({ params }: { params: Promise<{ category: string }> }) {
  ...
  const heading = cat ? `${catName(category, locale)} ${cat.emoji}` : 'Kansas Korean';
  const desc = cat ? pick(locale, cat.description) : '';
  const cta = pick(locale, T.cta);

  const fonts = await loadFonts(`${heading}${desc}${label}${cta}Kansas Korean`);
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', fontSize: 38 }}>
            ...
            <span style={{ marginLeft: 'auto', opacity: 0.6 }}>Kansas Korean</span>
          </div>
        </div>
      </div>
    ),
