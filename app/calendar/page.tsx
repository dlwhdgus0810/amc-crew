...
async function CalendarData() {
  ...
  /*
   * "오늘"은 서버가 정해서 내려준다 — 그 지역 시간대로.
   * 브라우저 시계는 다른 시간대일 수 있어서, 기기에 맡기면 하루 어긋난 칸에 동그라미가 그려진다.
   */
  ...
  const meetups = await listMeetupsBetween(region, from, to, user?.id, showPastPrivate);
  return <CalendarClient today={today} initial={{ from, to, meetups }} />;
}

/* 페이지 자체는 기다리는 것 없이 바로 그린다 — 지역·오늘은 안쪽(CalendarData)이 정한다 */
