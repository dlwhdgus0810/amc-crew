...
export function ProfileIcon({ season, size = 20 }: { season?: Season; size?: number }) {
  ...
  /* 단풍 — 잎 하나. 배경에 떨어지는 것은 도안 이미지(public/leaves)지만 20px짜리
     탭바 아이콘은 그 도안을 줄이면 갈색 얼룩이 된다. 여기서는 선으로 그린다.
     반지름 10의 호 둘을 마주 붙여 잎 모양을 만들고, 가운데 잎맥과 잎자루만 남겼다 —
     곁맥까지 그리면 20px에서 뭉개진다. */
  if (season === 'leaf') {
    return (
