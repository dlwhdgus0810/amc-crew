 ...
 * 적고, 없으면 도구 자체를 안 그린다. loginProviders도 같은 이유다 — 구글 키가 있는지는
 * 서버만 알고, 없으면 단추를 안 그려야 한다 (lib/google-oauth.ts의 enabledLoginProviders).
 */
interface RegionCtx {
  region: Region;
  amcName: string | null;
...
const Ctx = createContext<RegionCtx>({ region: DEFAULT_REGION, amcName: null, loginProviders: ['kakao'] });
...
/** 이 도메인에 그릴 로그인 단추 — 캔자스는 카카오 하나, 펜은 카카오·구글 */
export function useLoginProviders(): Provider[] {
  return useContext(Ctx).loginProviders;
}
