    ...
    class extends HTMLElement {
      ...
      step(dt) {
        ...
        for (let i = this.drops.length - 1; i >= 0; i--) {
          ...
          /*
           * 넘겨준 방울은 **줄기가 카드 위 선을 완전히 지날 때까지 여기서도 계속 그린다.**
           *
           * 넘기는 순간 지우면 카드 위 선 위쪽에 걸쳐 있던 줄기가 통째로 사라진다 —
           * 카드 캔버스는 자기 영역 밖을 못 그리기 때문이다. 그러면 길이 20~34px짜리
           * 줄기가 순간 토막으로 줄어서, 이어지는 게 아니라 튀어 보인다.
           *
           * 겹쳐 그려도 두 겹으로 안 보인다: 이 캔버스는 카드 뒤에 있어서 선 아래쪽은
           * 카드가 가리고, 카드 캔버스는 선 위쪽을 못 그린다. 둘이 정확히 나눠 맡는다.
           */
          if (d.hand) {
            if (d.y - d.len >= d.hand.top) {
              /*
               * 다시 나오는 자리는 **줄기의 위 끝**을 카드 아래 모서리에 맞춘다. 아래
               * 끝을 맞추면 방울이 제 길이만큼 더 내려간 자리에서 나오는데, 줄 사이
               * 틈이 20px 남짓이라 긴 방울(len은 7~34)은 그 한 걸음에 아랫줄 카드의
               * 위 선을 이미 지나쳐 버린다. 그러면 그 카드는 넘어서는 순간을 못 잡아
               * 아예 건너뛴다 — 윗줄은 130개를 받는데 아랫줄은 36개만 받았다.
               *
               * 줄마다 등장 애니메이션 지연이 달라 틈이 열별로 몇 px씩 다르다는 것이
               * 같은 줄 안에서도 76·55·36으로 갈리던 이유였다.
               */
              d.y = d.hand.bottom + (d.y - d.len - d.hand.top);
              d.hand = null;
            }
            continue;
          }
