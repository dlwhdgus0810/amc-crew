 ...
 *
 * 명단을 비우는 것은 **명단으로 모임을 만들었을 때뿐이다** — 모아 둔 사람들이 그 모임의
 * 참가자로 옮겨 갔으니 명단이 할 일이 끝난 것이다 (app/api/posts의 fromSignups).
 * 누가 목록에서 그냥 모임을 만든 경우에는 그대로 둔다. 그건 「독서나눔 할 사람」 명단이라
 * 다음 회차를 기다리는 중이고, 그 모임과는 상관이 없다. 빠지려면 본인이 신청을 취소한다.
 */
