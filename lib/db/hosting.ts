 ...
 *
 * 값을 매기지 않고 사람 수만 센다. 한 명당 몇 달란트인지는 lib/shop.ts가 정한다 —
 ...
 * 않겠느냐는 걱정은 있었는데, 한 명이면 5달란트라 모임을 열고 후기를 받는 수고에
...
const CONTRIB = {
  ...
  review: 3,
  /**
   * 건의함에 남긴 글 — **반영된 것만 센다** (status = 'done').
   *
   * 승인된 제안(7점)보다 낮게 둔 이유는 문턱이 다르기 때문이다. 카테고리 제안은
   * 이름·색까지 갖춰 내는 것이고, 건의는 한 줄이면 된다.
   *
   * **한때는 안 하기로 한 것까지 다 셌다.** 「적는 것만으로 값이 있다」가 이유였는데,
   * 그러면 적기만 하면 점수가 붙어서 셈이 헐거워진다 — 카테고리 제안이 승인된 것만
   * 세는 것과도 어긋났다. 반영된 것만 세면 두 규칙이 같아진다.
   *
   * 접수됨·반영 예정은 아직 안 센다. 반영 예정을 넣을지는 한 번 더 볼 만하다 —
   * 받아들여진 것은 맞는데 아직 화면에 없다.
   *
  ...
  const rows = resultRows(
    await db.execute(sql`
      WITH ok AS (
        ...
        SELECT user_id, count(*) AS n FROM tickets WHERE status = 'done' GROUP BY 1
      ), ids AS (
 ...
 * 뜻이 아니다. 상점의 달란트는 활동을 세는 것이라 여기서는 빼면 안 된다 — 빼면
 * 관리자만 영영 아무것도 못 산다.
 *
  ...
  const contribRows = resultRows(
    await db.execute(sql`
      WITH ok AS (
        ...
        SELECT count(*) AS n FROM tickets WHERE status = 'done' AND user_id = ${userId}
      )
 ...
 * 사람 수와 무관하게 세 번이다.
 *
 * 순위표(withProfiles)와 달리 관리자를 안 뺀다 — 달란트는 활동을 세는 것이라 표에
