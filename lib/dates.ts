...
// 지역 시간대 기준의 "오늘". Vercel 서버는 UTC로 돌기 때문에 서버 로컬 날짜를 쓰면
// 미국 중부 저녁 6~7시에 날짜가 넘어가 버린다 (오늘 저녁 모임이 시작 전에 "지난 모임"이 되는 버그).
//
// 시간대는 지역마다 다르다 (캔자스 Chicago, 필리 New_York). 그래서 시각이 걸리는 함수는
// 전부 지역을 **앞자리 필수 인자**로 받는다 — 기본값을 두면 빠뜨린 자리가 캔자스로 조용히
// 굳어서 필리 모임이 한 시간 어긋난다.
const tz = (region: Region) => REGIONS[region].timeZone;

/** 그 지역 기준 오늘 날짜 (YYYY-MM-DD) */
export function todayLocal(region: Region): string {
  // en-CA 로케일은 YYYY-MM-DD 형식을 그대로 반환한다
  return new Intl.DateTimeFormat('en-CA', { timeZone: tz(region) }).format(new Date());
}

export const WEEKDAYS = ['일', '월', '화', '수', '목', '금', '토'];

/**
 * 시각 하나를 그 지역 시간대의 'YYYY-MM-DDTHH:mm'으로 (datefmt의 entryLabel이 받는 모양).
 *
 * 시간대 변환은 서버에서 한 번만 한다 — 브라우저에 UTC를 그대로 넘기면
 * 다른 시간대에서 열었을 때 화면에 뜨는 시각이 사람마다 달라진다.
 */
export function localStamp(region: Region, at: Date): string {
...
/**
 * "지난 모임" 판정 기준 시각을 그 지역 시간대의 (날짜, 시각)으로 반환.
 * 현재 시각에서 유예만큼 뺀 값이라, 모임의 (date, endTime)이 이보다 크면 아직 예정이다.
 */
export function pastCutoff(region: Region): { date: string; time: string } {
  const at = new Date(Date.now() - PAST_GRACE_MINUTES * 60_000);
  return {
    date: new Intl.DateTimeFormat('en-CA', { timeZone: tz(region) }).format(at),
 ...
 */
export function openEndCutoffTime(region: Region): string | null {
  const cut = toMinutes(pastCutoff(region).time) - OPEN_END_MINUTES;
...
/**
 * 그 지역 시간대의 벽시계 시각(날짜 + 'HH:mm')이 가리키는 실제 순간.
 *
 ...
 */
export function instantAt(region: Region, date: string, hhmm: string): Date {
  const wall = Date.parse(`${date}T${hhmm}:00Z`);
  let guess = wall;
  for (let i = 0; i < 2; i++) {
    guess = wall + (guess - Date.parse(`${localStamp(region, new Date(guess))}:00Z`));
  }
